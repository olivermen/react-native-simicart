...
class ProductsPage extends SimiPageComponent {
    constructor(props) {
        ...
        this.manufacturer_option_id = this.props.navigation.state.params.hasOwnProperty("manufacturer_option_id") ? this.props.navigation.getParam("manufacturer_option_id") : null;
        this.manufacturer_name = this.props.navigation.state.params.hasOwnProperty("manufacturer_name") ? this.props.navigation.getParam("manufacturer_name") : null;
        this.manufacturer_image = this.props.navigation.state.params.hasOwnProperty("manufacturer_image") ? this.props.navigation.getParam("manufacturer_image") : null;
        this.categoryName = this.props.navigation.getParam("categoryName") ? this.props.navigation.getParam("categoryName") : null;
    }

    componentWillMount() {
        this.setState({ showLoading: 'full' })
    }
    ...
    requestData(params) {
        let url = products;
        if (this.manufacturer_option_id) {
            url = 'simiconnector/rest/v2/manufacturers/' + this.manufacturer_option_id
        }
    ...
    checkExistData() {
        ...
        if (this.query || this.manufacturer_option_id) {
            return false;
        }
        switch (this.mode) {
            case products_mode.spot:
    ...
    onFilterAction(filterParams) {
        ...
        this.setState({ data: null, showLoading: 'dialog', showFilter: false });
    ...
    onSortAction(order, dir) {
        ...
        this.setState({ data: null, showSort: false, showLoading: 'dialog' });
        this.requestData(params);
    }
    ...
    renderPhoneLayout() {
        ...
        return (
            <Container style={{ backgroundColor: variable.appBackground }}>
                {contentLayout.length > 0 && <Content contentContainerStyle={{ paddingBottom: verticalScale(40) }}>
                    <View>
                        {contentLayout}
                    </View>
                </Content>}
