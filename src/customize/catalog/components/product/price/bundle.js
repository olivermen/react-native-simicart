...
class Bundle extends React.Component {
    ...
    renderPriceFromToWithoutTax() {
        const { prices, screen } = this
        let price = null
        let original_price = null

        if (prices.regular_price_display > 0 && prices.regular_price_display < 100 && screen !== 'Products') {
            original_price = (
                <Format
                    price={screen === 'SearchProducts' ? prices.from_price / (1 - (prices.regular_price_display / 100)) : prices.ori_price}
                    style={this.parent.style.stylePriceLine}
                />
            )
        }

        if (prices.price) {
            price = <Format price={prices.price} style={this.parent.style.stylePrice} />
        } else {
            price = (
                <Format
                    price={screen === 'SearchProducts' || screen === 'Products' ? prices.from_price : prices.price_after_discount_display}
                    style={this.parent.style.stylePrice}
                />
            )
        }
