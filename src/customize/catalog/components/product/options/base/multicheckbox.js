...
class MultiCheckbox extends Abstract {
    ...
    renderForBundle = () => {
        ...
        for (let i in options) {
            ...
            let label = this.parent.renderLabelOption(item.name, price, item.qty, app_tier_prices, item.is_salable === 'OUT_OF_STOCK');
            console.log('l;aa', item);
            let element = (
                <Checkbox
                    key={i}
                    label={label}
                    disabled={item.is_salable === 'OUT_OF_STOCK' ? true : false}
                    selected={selected}
                    value={i}
                    parent={this.parent}
